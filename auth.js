(function () {
    ...
    function loginUser(email, password) {
        ...
        return hashPassword(password).then(function (hash) {
            ...
            localStorage.setItem(SESSION_KEY, JSON.stringify({ email: norm }));
            return { ok: true };
        });
    }
    ...
    function getSession() {
        try {
            var raw = localStorage.getItem(SESSION_KEY) || sessionStorage.getItem(SESSION_KEY) || sessionStorage.getItem(LEGACY_SESSION_KEY);
            if (!raw) return null;
    ...
    function getSafeNextUrl(paramValue) {
        if (!paramValue) return "panel.html";
        try {
            var decoded = decodeURIComponent(paramValue);
        } catch (e) {
            return "panel.html";
        }
        if (decoded.indexOf("..") !== -1) return "panel.html";
        var allowed = ["panel.html", "index.html"];
